...
public class GremlinQueryComposerTest {
    ...
    @Test
    public void HASRrealtionshipTest() {
        String expected =
                "g.V().has('__typeName', 'DB').as('d').has('DB.name', eq('db')).bothE().bothV().has('__typeName','Table').as('t').dedup().limit(25).toList()";
        verify("from DB as d where d.name='db' hasr Table as t", expected);
