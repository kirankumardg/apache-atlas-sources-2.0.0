...
public class DSLVisitor extends AtlasDSLParserBaseVisitor<Void> {
    ...
    @Override
    public Void visitHasRExpression(HasRExpressionContext ctx) {
        if (LOG.isDebugEnabled()) {
            LOG.debug("=> DSLVisitor.visitHasRExpression({})", ctx);
        }
        gremlinQueryComposer.addHasRClause(ctx.hasRClause().identifier().getText());
        if(ctx.identifier() != null ) {
            String identifier = ctx.identifier().getText();
            if (LOG.isDebugEnabled()) {
                LOG.debug("=> DSLVisitor.visitHasRClause.identifier({})", identifier);
            }

            gremlinQueryComposer.addAsClause(ctx.identifier().getText());
        }
        return super.visitHasRExpression(ctx);
    }
